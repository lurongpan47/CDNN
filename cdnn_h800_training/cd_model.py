...
Model configurations:
  - CD-Transformer-Small   (~125M params, ~33M CD-effective)
  - CD-Transformer-Medium  (~350M params, ~90M CD-effective)
  - CD-Transformer-Large   (~1.3B params, ~350M CD-effective)
...
@dataclass
class CDModelConfig:
    """Configuration for CD-Transformer model.

    Design choices motivated by cost-reduction synergies:
      - CD block size B: larger → more compression but less expressivity
        Use B ∈ {3, 5, 7} per CD theory (polygon multiplicities 2L+1)
      - MoE: n_experts × n_active controls compute/capacity trade-off
        DeepSeek-V3 uses 256 experts / 8 active; we scale down for H800×8
      - MLA kv_lora_rank: latent KV dimension (compression ratio = dim/rank)
      - MTP: multi-token prediction heads for training signal density
    """
    # Model dimensions
    ...
    # Training
    use_fp8: bool = True        # FP8 mixed precision
...
class MTPHead(nn.Module):
    ...
    def forward(self, hidden: torch.Tensor,
        ...
        """
        # Concatenate hidden + shifted embeddings, project down
...
class CDTransformer(nn.Module):
    ...
    Cost reduction breakdown (vs dense transformer):
      ┌─────────────────────────────────────────────────────┐
      ...
      │  FP8 training       │ ~2× throughput on H800       │
        ...
        if labels is not None:
            ...
            if self.mtp_heads is not None and self.training:
                ...
                for depth, mtp_head in enumerate(self.mtp_heads):
                    # Shift embeddings for next-next token prediction
                    shift = depth + 2  # predict token at position +2, +3, ...
                    if T > shift:
                ...
                if mtp_losses:
                    # Weight MTP losses (DeepSeek-V3 uses equal weighting)
...
if __name__ == "__main__":
    ...
    for size in ["small"]:
        ...
        for m in model.modules():
            if isinstance(CDLinear, type(m)):
                pass  # CDLinear check
            if hasattr(m, 'condition_number') and callable(m.condition_number):
                try:
                    k = m.condition_number()
                    if not math.isnan(k):
                        kappas.append(k)
                except:
                    pass
        if kappas:
            import numpy as np
            print(f"\n  Hessian condition numbers (CDLinear layers):")
            print(f"    Mean κ: {np.mean(kappas):.2f}")
            print(f"    Max  κ: {np.max(kappas):.2f}")
            print(f"    Min  κ: {np.min(kappas):.2f}")
